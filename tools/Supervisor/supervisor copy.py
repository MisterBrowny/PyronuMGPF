...
GLOBAL_MAP = {
    0: ("END",    "gray-700 font-bold"),
    1: ("GO",     "green-600 font-bold"),
    2: ("ARMED",  "blue-600 font-bold"),
    3: ("TEST",   "orange-600 font-bold"),
}

ANALOG_MAP = {
    0b00: ("ABSENT", "text-gray-500"),
    0b01: ("KO",     "text-red-600 font-bold"),
    0b10: ("MOYEN",  "text-yellow-600 font-bold"),
    0b11: ("OK",     "text-green-600 font-bold"),
}

DEFAULT_GLOBAL = ("???", "text-purple-600 font-bold")
DEFAULT_ANALOG = ("ERR", "text-red-600 font-bold")
...
@ui.page('/')
async def main_page():
    ui.label('Supervision Modbus RTU – 20 appareils').classes('text-2xl font-bold text-center mt-4 mb-1')
    ...
    with ui.tabs().classes('w-full text-lg') as tabs:
        ...
        for dev in range(1, NUM_DEVICES + 1):
            sid = SLAVE_IDS[dev - 1]
            tab_list.append(ui.tab(f'App {dev:02d} – ID {sid}'))
        ...
        for dev_idx, tab in enumerate(tab_list, start=1):
            ...
            with ui.tab_panel(tab):
                with ui.card().classes('w-full'):
                    ui.label(f'Appareil {dev_idx} • Esclave {sid}').classes('text-xl font-semibold mb-4')

                    # État global
                    with ui.row().classes('items-center gap-4 mb-5'):
                        ui.label('État :').classes('text-lg')
                    ...
                    # Analogiques
                    ui.label('Entrées analogiques').classes('text-lg mt-4 mb-2')
...
if __name__ in {"__main__", "__mp_main__"}:
    ui.run(
        title='Supervision Modbus 20 esclaves',
        host='0.0.0.0',
        port=8080,
        dark=True,
        reload=True,

...
async def main_page():
    ...
    with ui.tabs().classes('w-full text-2xl') as tabs:
        tab_list = [ui.tab(f'PYRO {dev:02d} – ID {sid}') for dev, sid in enumerate(SLAVE_IDS, 1)]
    ...
    global_status = ui.label('Auto-refresh toutes les 0.1s').classes('text-center mt-8 text-lg')
